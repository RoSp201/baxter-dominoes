...
class DominoService:
  ...
  def imgReceived(self, message):
    ...
    self.analyzeImage([50000, [30, 35], [10, 50, 25, 0 ,35]])
  ...
  def analyzeImage(self, params):
    ...
    orig = cv_bridge.CvBridge().imgmsg_to_cv2(self.lastImage, desired_encoding="bgr8")
    ...
    for i in range(len(contours)):
        ...
        if w*h < 10000000 and w*h > 50000 and (abs(2*w - h) > 20 or abs(2*h - w) > 30) and (abs(abs(angle) - 90) < 30 or abs(angle) < 20) :
